        ...
        <div className="">
            <section className="px-4 py-24 tags -mt-8">
                <div className="container mx-auto">
                    <Breadcrumb item={type.toLowerCase()} />
                    ...
                    {loading ? (<Loader />) : (
                        <Fragment>
                            <p className="mb-8">
                                {insightList.length} <span className="opacity-50">results found</span>
                            </p>
                            {insightList.length ? (
                                <div className="grid grid-cols-3 gap-x-10 gap-y-10 card">
                                    {insightList.map((insight) => {
                                        return (
                                            <Link className="card__item"
                                                  to={`/${insight.contents_type.toLowerCase()}/${insight.id}`}
                                                  key={insight.id}>
                                                {get(
                                                    insight,
                                                    'content_image.meta.download_url'
                                                ) && (
                                                    <div className="img-wrapper " style={{ height: '240px' }}>
                                                ...
                                                <div className="card__caption  text-blue-50  mt-4">
                                                    <label className="text-sm bg-primary-gray px-3 py-1 rounded-xl">{tag}</label>
