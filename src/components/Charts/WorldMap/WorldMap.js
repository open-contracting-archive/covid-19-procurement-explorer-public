...
const WorldMap = ({ data }) => {
    ...
    useLayoutEffect(() => {
        ...
        am4core.ready(function () {
            ...
            // Bubble series
            let bubbleSeries = mapChart.series.push(
            ...
            // switch between map and globe
            let absolutePerCapitaSwitch = mapChart.createChild(
            ...
            absolutePerCapitaSwitch.events.on('toggled', function () {
                if (absolutePerCapitaSwitch.isActive) {
                    ...
                    polygonSeries.heatRules.getIndex(0).max =
                        colors[currentType]
            ...
            let buttonsAndChartContainer = container.createChild(
                am4core.Container
            )
            ...
            sizeSlider.events.on('rangechanged', function () {
                ...
                bubbleSeries.heatRules.getIndex(0).max =
                    30 + sizeSlider.start * 100
            ...
            let lineChart = chartAndSliderContainer.createChild(
                am4charts.XYChart
            )
            ...
            dateAxis.max =
                lastDate.getTime() + am4core.time.getDuration('day', 5)
  ...
  dateAxis.renderer.labels.template.adapter.add("fillOpacity", function(fillOpacity, target){
      return dateAxis.valueToPosition(target.dataItem.value) + 0.1;
            ...
            lineChart.legend.itemContainers.template.events.on(
                'down',
                function () {
                    legendDown = true
                }
            ...
            let seriesTypeSwitch = lineChart.legend.createChild(
                am4core.SwitchButton
            )
            ...
            function createColumnSeries() {
                ...
                columnSeries.confirmed = addColumnSeries(
                    'confirmed',
                    confirmedColor
                )
                columnSeries.recovered = addColumnSeries(
                    'recovered',
                    recoveredColor
                )
            ...
            function addButton(name, color) {
                ...
                let activeHoverState = button.background.states.create(
                    'hoverActive'
                )
            ...
            function changeDataType(name) {
                ...
                bubbleSeries.dataItems.each(function (dataItem) {
                    dataItem.setValue(
                        'value',
                        dataItem.dataContext[currentType]
                    )
                })
                ...
                polygonSeries.dataItems.each(function (dataItem) {
                    dataItem.setValue(
                        'value',
                        dataItem.dataContext[currentType + 'PC']
                    )
            ...
            function rollOverCountry(mapPolygon) {
                ...
                if (mapPolygon) {
                    ...
                    let image = bubbleSeries.getImageById(
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    if (image) {
                        let population = Number(
                            populations[image.dataItem.dataContext.id]
                        )
                    ...
                    if (polygon) {
                        ...
                        polygon.dataItem.dataContext.activePC =
                            (polygon.dataItem.dataContext.active / population) *
                            1000000
                    ...
                    bubbleSeries.heatRules.getIndex(0).maxValue =
                        max[currentType]
                    polygonSeries.heatRules.getIndex(0).maxValue =
                        maxPC[currentType]
            ...
            function handleImageOver(event) {
                rollOverCountry(
                    polygonSeries.getPolygonById(event.target.dataItem.id)
                )
            }
